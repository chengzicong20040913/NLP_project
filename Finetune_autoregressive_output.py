...
def finetune():
    ...
    def data_collator(batch: List[Dict]):
        ...
        for i in range(len(input_ids)):
            ...
            mask = torch.ones(length, length, dtype=torch.long)
            mask = torch.tril(mask)  # 生成下三角矩阵
            causal_attention_mask.append(mask)
        attention_mask = tokenized_inputs.attention_mask
        labels = [
            [-100 if token == tokenizer.pad_token_id else token for token in label]
            for label in tokenized_inputs.input_ids
        ]
        labels = [
            [-100] + label[:-1]  # right shift by 1 for output part
            for label in labels
        ]
        for i in range(len(labels)):
            # 假设前半部分是输入，后半部分是输出
            # 将输入部分的 `labels` 设置为 -100，确保只计算输出部分
            input_length = len(batch[i]['instruction'] + batch[i]['input'])+1  # 输入部分的长度
            labels[i][:input_length] = [-100] * input_length
        #要转换为torch格式
        input_ids = torch.tensor(input_ids, dtype=torch.long)
        attention_mask = torch.tensor(attention_mask, dtype=torch.long)
        labels = torch.tensor(labels, dtype=torch.long)
        causal_attention_mask = torch.stack(causal_attention_mask)
